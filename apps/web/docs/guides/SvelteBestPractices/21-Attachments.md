...
<button {@attach tooltip('With options', { placement: 'bottom' })}>
  Hover me too
</button>
```
...
<!-- Use converted action as attachment -->
<p {@attach colorize({ color })}>
  Colored text
</p>
```
...
function fromAction<T>(
  action: (node: HTMLElement, params: T) => {
...
{#if open}
  <div {@attach clickOutside(() => open = false)} class="dropdown">
    Dropdown content
  </div>
{/if}
```
...
<div {@attach observeResize((entry) => {
