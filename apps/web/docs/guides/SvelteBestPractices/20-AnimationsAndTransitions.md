...
<!-- In/out separately -->
{#if visible}
	<div in:fly={{ y: 200, duration: 300 }} out:fade={{ duration: 200 }}>Flies in, fades out</div>
{/if}
...
{#if show}
	<p transition:typewriter={{ speed: 2 }}>This text will type out character by character</p>
{/if}
```
...
<div style="width: {smoothValue.current}px">Smoothly resizes</div>
```
...
<div style="width: {smoothSize.current}px; height: {smoothSize.current}px">Bouncy resize</div>
```
...
<button onclick={() => (color.target = '#0000ff')}> Blue </button>

<div style="background: {color.current}">Smooth color transition</div>
```
