...
<script lang="ts">
	...
	let greeting = $derived(formal ? `Good day, ${name}.` : `Hey ${name}!`);
</script>
...
describe('Greeting', () => {
	it('renders informal greeting by default', async () => {
		...
		await expect.element(page.getByTestId('greeting')).toHaveTextContent('Hey World!');
	});

	it('renders formal greeting when specified', async () => {
		render(Greeting, { name: 'Mr. Smith', formal: true });

		await expect.element(page.getByTestId('greeting')).toHaveTextContent('Good day, Mr. Smith.');
	});
});
```
...
<form onsubmit={handleSubmit}>
	<input type="search" bind:value placeholder="Search..." aria-label="Search" />
...
describe('SearchInput', () => {
	...
	it('uses initial value', async () => {
		...
		await expect.element(page.getByLabel('Search')).toHaveValue('initial query');
	});
});
```
...
describe('Card', () => {
	...
	it('renders header when provided', async () => {
		...
		await expect.element(page.getByRole('heading', { name: 'Card Title' })).toBeVisible();
	});
});
```
...
describe('Theme Context', () => {
	it('provides theme to children', async () => {
		...
		await expect.element(page.getByTestId('theme-display')).toBeVisible();
	});
...
it('has correct form action', async () => {
	...
	await expect.element(page.getByRole('form')).toHaveAttribute('action', '/api/contact');
});
