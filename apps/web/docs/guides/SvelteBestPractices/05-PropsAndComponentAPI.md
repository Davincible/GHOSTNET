...
<button class="btn btn-{variant}" class:loading disabled={disabled || loading} {...rest}>
	{#if loading}
		<Spinner />
	{/if}
	{@render children?.()}
...
<Button variant="primary" type="submit" aria-label="Save" onclick={handleClick}>Save</Button>
```
...
<script lang="ts">
	...
	let { class: className = '', for: htmlFor, 'aria-label': ariaLabel } = $props();
</script>
