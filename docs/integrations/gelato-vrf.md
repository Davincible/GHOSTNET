# Gelato VRF Integration Guide

> **Status**: Alternative/Fallback option. GHOSTNET core game uses **prevrandao + 60s lock period** for trace scans (see [ADR-001](../decisions/ADR-001-randomness-strategy.md)). Arcade games use **future block hash** pattern. This guide documents Gelato VRF as a potential future option if external VRF becomes necessary.
...
## Why GHOSTNET Uses Block-Based Randomness Instead

For the core trace scan mechanic, we chose prevrandao + lock period over VRF because:

1. **Zero latency** — VRF adds 1500ms delay; prevrandao is instant
2. **Zero cost** — VRF has gas + premium costs; prevrandao is free
3. **No external dependency** — VRF requires Gelato service availability
4. **Same trust model** — Both trust a single party (sequencer vs Gelato operator)

The 60-second lock period eliminates the front-running window, making prevrandao acceptable for our use case. See `docs/learnings/001-prevrandao-megaeth.md` for detailed analysis.
