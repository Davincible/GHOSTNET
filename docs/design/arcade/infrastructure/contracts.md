...
|              v            v             v             v            v                      |
|   +----------+--+ +-------+-----+ +-----+------+ +------------+ +-+------------+         |
|   | GameRegistry| | PayoutMgr   | | FeeRouter  | | BlockRand  | | RateLimiter  |         |
...
contract HashCrashIntegrationTest is Test {
    ...
    function test_FullGameCycle() public {
        ...
        // 4. Game crashes
        vm.warp(block.timestamp + 30);  // Future block hash determines crash
...
│   └── RandomnessLib.sol             # Future block hash helpers
└── storage/
